...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                ...
                archiveArtifacts(
                    artifacts: 'reports/image-*.tar.gz,reports/image-*.txt',
                    fingerprint: true
                )
            }
        }
            ...
            steps {
                sh '''
                    ...
                    ruff check app tests scripts \
                        --select E4,E7,E9,F \
                        --output-format json > reports/ruff.json

                    python scripts/quality_gate.py
                '''

                archiveArtifacts(
                    artifacts: 'reports/ruff.json,reports/complexity.json',
                    fingerprint: true
                )
            }
        }
        ...
        stage('Security') {
            steps {
                sh '''
                    ...
                    pip-audit \
                        ...
                        -f json \
                        -o reports/dependencies.json

                    trivy image \
                        --exit-code 1 \
                        --severity HIGH,CRITICAL \
                        --format json \
                        --output reports/container-security.json \
                        "$APP_IMAGE"
                '''
            }
            ...
            post {
                always {
                    archiveArtifacts(
                        allowEmptyArchive: true,
                        artifacts: 'reports/bandit.json,reports/dependencies.json,reports/container-security.json'
                    )
                }
            }
        }
        ...
        stage('Deploy (staging)') {
            steps {
                sh '''
                    ...
                    docker compose -p taskpulse-staging \
        ...
        stage('Release (production)') {
            steps {
                sh '''
                    ...
                    docker compose -p taskpulse-monitor \
                ...
                archiveArtifacts(
                    artifacts: 'reports/release-version.txt,release-history.txt',
                    allowEmptyArchive: true
                )
            }
        }
            ...
            steps {
                sh '''
                    ...
                    for i in $(seq 1 30); do
                        if python3 scripts/check_monitoring.py; then
                            exit 0
                        fi
    ...
    post {
        success {
            echo 'All seven assessment stages passed. Incident simulation can be demonstrated separately.'
        }

        failure {
            echo 'Pipeline failed; inspect the failing stage and archived reports before promotion.'
        }
